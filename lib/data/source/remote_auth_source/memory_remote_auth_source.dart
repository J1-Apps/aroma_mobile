...
class MemoryRemoteAuthSource extends MemorySource implements RemoteAuthSource {
  final _stream = BehaviorSubject<SessionModel>.seeded(SessionModelSignedOut());
  ...
  Future<void> createUserWithEmailAndPassword(String email, String password) async {
    await wrapRequest(
      Future.sync(() => _stream.add(const SessionModelSignedIn(userId: "1"))),
  ...
  Future<void> signInWithGoogle() async {
    await wrapRequest(
      Future.sync(() => _stream.add(const SessionModelSignedIn(userId: "2"))),
  ...
  Future<void> signOut() async {
    await wrapRequest(
      Future.sync(() => _stream.add(const SessionModelSignedOut())),
